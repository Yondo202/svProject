  ...
  return (
    <Conainer className="container">
      <div className="Header">
        <Linka to="/admin" href="/admin" >
          <div className="backPar">
            <h2 className="one">{`<`}</h2>
            <h2 className="tow">{`<`}</h2>
          </div>
        </Linka>
        <h3 ref={ref} >{props.all.parent} {props.all.name}</h3>
      ...
      <div className="btnPar">
        ...
        {window? (<Pdf targetRef={ref} filename="post.pdf">
          {({ toPdf }) => <button onClick={toPdf} >Print Click me</button>}
        </Pdf>) : false}
        {/* {process.browser && (<Pdf targetRef={ref} filename="post.pdf">
          {({ toPdf }) => <button onClick={toPdf} >Print Click me</button>}
        </Pdf>)} */}
