...
function Forms() {
    ...
    const [PrevBtn, setPrevBtn] = useState('flex')
    const [NextBtn, setNextBtn] = useState('block')
    const [Submit, setSubmit] = useState('flex')
    ...
    return (
        <Conainer  className="container">
          ...
          <div className="btnPar">
            <button tabIndex={1} onClick={clickHandler} style={{display:`${PrevBtn}`}}>Click me previous</button>
            <button tabIndex={2} onClick={clickHandler} style={{display:`${NextBtn}`, justifySelf:"flex-end"}}>Click me next</button>
            <button tabIndex={3} onClick={clickHandler} style={{display:`${Submit}`, justifySelf:"flex-end"}}>Илгээх</button>
          </div>
